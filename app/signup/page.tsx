...
import { Eye, EyeOff, Mail, Lock, User, Loader2, Brain } from "lucide-react";
import { apiRegister, saveToken, saveUser, loginWithGoogle, apiLogin, loginWithFacebook, loginWithLinkedIn } from "@/lib/api";
import { useAuth } from "@/context/AuthContext";

/* ── Animated Neon Brain Orb (shared with login) ── */
function BrainOrb() {
  return (
    <>
      <style>{`
...
export default function SignupPage() {
  ...
  const strength = (() => {
    let sc = 0;
    if (form.password.length >= 8) sc++;
    if (/[A-Z]/.test(form.password)) sc++;
    if (/[0-9]/.test(form.password)) sc++;
    if (/[^A-Za-z0-9]/.test(form.password)) sc++;
    return sc;
  })();
  ...
  return (
    <div style={s.root}>
      ...
      <div style={s.right}>
        <div style={s.box}>
          <div style={s.tabs}>
            <Link href="/login" style={s.tab}>Sign In</Link>
            <Link href="/signup" style={{...s.tab,...s.tabOn}}>Sign Up</Link>
          </div>

          <h2 style={s.title}>Get started free 🚀</h2>
          <p style={s.desc}>Create your GrowthOS account today</p>
          ...
          <form onSubmit={handleSubmit} style={s.form}>
            ...
            <div style={s.field}>
              ...
              <div style={s.wrap}>
                ...
                <input type={showPwd ? "text" : "password"} placeholder="Min. 8 characters"
                  ...
                  style={{...s.input, paddingRight:"42px"}} required />
              ...
              {form.password.length > 0 && (
                <div style={{ display:"flex", alignItems:"center", gap:"8px", marginTop:"5px" }}>
                  <div style={{ display:"flex", gap:"4px", flex:1 }}>
                    {[1,2,3,4].map(i => (
                      <div key={i} style={{ height:"4px", flex:1, borderRadius:"2px", transition:"background .3s", background: i <= strength ? strColors[strength] : "#e0e7f0" }}/>
                    ))}
                  </div>
                  <span style={{ fontSize:"0.72rem", fontWeight:600, color:strColors[strength] }}>{strLabels[strength]}</span>
                </div>
              )}
            </div>
            ...
            <div style={{ display:"flex", alignItems:"flex-start", gap:"9px" }}>
              ...
              <label htmlFor="terms" style={{ fontSize:"0.78rem", color:"#6b7a99", lineHeight:1.5 }}>
                I agree to the <Link href="/terms" style={{ color:"#1565c0", textDecoration:"none" }}>Terms of Service</Link> and <Link href="/privacy" style={{ color:"#1565c0", textDecoration:"none" }}>Privacy Policy</Link>
              </label>
            </div>

            <button type="submit" style={s.btnMain} disabled={loading}>
              {loading
                ? <span style={s.row}><Loader2 size={17} style={s.spin}/> Creating account...</span>
                : "CREATE MY ACCOUNT"}
            </button>
          </form>

          <div style={s.divider}>
            <div style={s.line}/><span style={s.or}>or sign up with</span><div style={s.line}/>
          </div>

          <div style={s.socials}>
            <button style={s.socialBtn} onClick={loginWithGoogle}>
            ...
            <button style={s.socialBtn} onClick={loginWithFacebook}>
              <svg width="18" height="18" viewBox="0 0 24 24">
                <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" fill="#1877F2"/>
              </svg>
              <span>Facebook</span>
            </button>
            <button style={s.socialBtn} onClick={loginWithLinkedIn}>
              <svg width="18" height="18" viewBox="0 0 24 24">
...
const s: Record<string, any> = {
  ...
  orange:    { color:"#ff6b00" },
  headline:  { fontFamily:"'Rajdhani','Segoe UI',sans-serif", fontSize:"1.5rem", fontWeight:700, color:"#fff", lineHeight:1.15, marginBottom:"12px", marginTop:"16px", "@media (min-width: 768px)": { fontSize:"2rem" } },
  sub:       { fontSize:"0.85rem", color:"#b0bec5", lineHeight:1.7, marginBottom:"24px", "@media (min-width: 768px)": { fontSize:"0.93rem", marginBottom:"32px" } },
  stats:     { display:"flex", gap:"8px", justifyContent:"center", flexDirection:"column", "@media (min-width: 640px)": { flexDirection:"row", gap:"12px" } },
  ...
  statNum:   { fontFamily:"'Rajdhani',sans-serif", fontSize:"1.25rem", fontWeight:700, color:"#2196f3", "@media (min-width: 768px)": { fontSize:"1.5rem" } },
  statLbl:   { fontSize:"0.65rem", color:"#b0bec5", textTransform:"uppercase", letterSpacing:"0.1em" },
  right:     { width:"100%", background:"linear-gradient(145deg,#fff 0%,#e8f0fe 100%)", display:"flex", alignItems:"center", justifyContent:"center", padding:"30px 20px", overflowY:"auto", "@media (min-width: 768px)": { width:"48%", padding:"50px 40px" } },
  box:       { width:"100%", maxWidth:"400px" },
  tabs:      { display:"flex", background:"#e0e7f0", borderRadius:"12px", padding:"5px", marginBottom:"24px" },
  tab:       { flex:1, textAlign:"center", padding:"10px", fontSize:"0.85rem", fontWeight:600, color:"#6b7a99", textDecoration:"none", borderRadius:"9px", transition:"all .25s", "@media (min-width: 768px)": { fontSize:"0.9rem" } },
  tabOn:     { background:"white", color:"#1565c0", boxShadow:"0 2px 8px rgba(0,0,0,.12)" },
  title:     { fontFamily:"'Rajdhani',sans-serif", fontSize:"1.5rem", fontWeight:700, color:"#0d1f3c", marginBottom:"4px", "@media (min-width: 768px)": { fontSize:"1.9rem" } },
  desc:      { fontSize:"0.8rem", color:"#7a8aa0", marginBottom:"20px", "@media (min-width: 768px)": { fontSize:"0.85rem", marginBottom:"24px" } },
  form:      { display:"flex", flexDirection:"column", gap:"12px", "@media (min-width: 768px)": { gap:"14px" } },
  nameRow:   { display:"flex", gap:"10px", "@media (min-width: 768px)": { gap:"12px" } },
  ...
  btnMain:   { width:"100%", padding:"13px", background:"linear-gradient(135deg,#1565c0 0%,#1e88e5 100%)", border:"none", borderRadius:"10px", fontSize:"0.9rem", fontWeight:700, letterSpacing:"0.06em", color:"white", cursor:"pointer", transition:"all .25s", boxShadow:"0 4px 15px rgba(21,101,192,.35)", fontFamily:"'Rajdhani',sans-serif", "@media (min-width: 768px)": { fontSize:"0.95rem" } },
  row:       { display:"flex", alignItems:"center", justifyContent:"center", gap:"8px" },
  spin:      { animation:"spin .8s linear infinite" },
  divider:   { display:"flex", alignItems:"center", gap:"12px", margin:"16px 0", "@media (min-width: 768px)": { margin:"20px 0" } },
  line:      { flex:1, height:"1px", background:"#d0dbe8" },
  or:        { fontSize:"0.75rem", color:"#9aabb8", whiteSpace:"nowrap", fontWeight:500 },
  socials:   { display:"flex", gap:"8px", "@media (min-width: 768px)": { gap:"10px" } },
  socialBtn: { flex:1, display:"flex", alignItems:"center", justifyContent:"center", gap:"6px", padding:"9px 6px", border:"1.5px solid #d0dbe8", borderRadius:"10px", background:"white", fontSize:"0.75rem", fontWeight:600, color:"#1a2740", cursor:"pointer", transition:"all .25s", fontFamily:"inherit", "@media (min-width: 768px)": { fontSize:"0.8rem", gap:"7px", padding:"10px 8px" } },
  switch:    { textAlign:"center", marginTop:"16px", fontSize:"0.78rem", color:"#7a8aa0", "@media (min-width: 768px)": { marginTop:"20px", fontSize:"0.82rem" } },
